...
class Library:
    ...
    def remove_book(self, id_books: int) -> None:
        """
    ...
    def print_all_books(self) -> None:
        ...
        return self.session.query(Book).order_by(Book.id.asc())
    ...
    def print_list_books_available(self) -> None:
        ...
        #     print(instance)
        return self.session.query(Book).order_by(Book.id.asc()).where(Book.id_reader == None)
    ...
    def give_book(self, id_books: int, id_reader: int) -> None:
        """
        ...
        for id_book in id_books:
            ...
            if id_book not in obj_book:
                ...
                return
            obj_book2 = [book.id for book in self.session.query(Book).where(Book.id_reader != None)]
    ...
    def return_book(self, id_books: int, id_reader: int) -> None:
        """
    ...
    def print_list_books_user(self, id_user) -> None:
        """
        Выводит список книг, которые находятся на руках у читателей

        :return: None
        """
        return self.session.query(Book).where(Book.id_reader == id_user)

    def get_reader_email(self, email_: str):
        """
        Проверяет на наличие регестрации емейл

        :return: None
        """
        for instance in self.session.query(Reader).where(Reader.email == email_):
            return instance

        # all_readers = self.session.query(Reader)
        # for reader in all_readers:
        #     if reader.email == email_:
        #         print('Email уже есть')
        #         return reader
        #
        # print('email в базе нет')
        # return None

    def get_reader_id(self, id: int):
        """
        :return: None
        """
        for instance in self.session.query(Reader).where(Reader.id == id):
            return instance
